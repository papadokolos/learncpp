...
namespace V3
{
    // Automatic padding with C++23 prints
    void printNumbersDownTo1(int max_number);
}
...
namespace V2
{
    ...
    {
        std::print("{}", std::string(static_cast<std::size_t>(pad_size), ' '));
    }
...
namespace V3
{
    ...
    void printNumbersDownTo1(int max)
    {
        ...
        while (outer <= max)
        {
            ...
            // I resort to ugly solution:
            std::print(""); // Insert padding
            for (auto num : std::views::iota(1, max + 1))
            {
                std::print(" {}", num ); // Numbers
            }
            std::println();
            ++outer;
        }
